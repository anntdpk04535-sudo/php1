...
try {
    ...
    // Tự động tạo bảng đơn hàng nếu chưa có
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'place_order') {
        ...
        try {
            ...
            $stmtOrder = $pdo->prepare("INSERT INTO orders (order_id, fullname, phone, email, address, note, payment, total, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmtOrder->execute([
                $orderId,
                $fullname,
                $phone,
            ...
            // Lưu session last_order phục vụ việc hiển thị popup thành công ngay sau khi đặt
            $_SESSION['last_order'] = [
                'order_id'   => $orderId,
                'fullname'   => $fullname,
                'phone'      => $phone,
            ...
            $pdo->rollBack();
            echo json_encode(['ok' => false, 'errors' => ['server' => 'Lỗi kết nối CSDL khi lưu đơn hàng: ' . $e->getMessage()]]);
            exit;
        }
    }
}
      ...
      <div class="form-group">
        ...
        <div class="pay-options">
          ...
          <label class="pay-option">
            ...
            <span class="pay-label">MoMo</span>
          </label>
        </div>
  ...
  cart.forEach(item => {
    ...
    rows += `
      <tr>
        ...
        <td>
          <button class="cart-remove" onclick="removeItem('${escHtml(item.product_id)}')">Xóa</button>
        </td>
      </tr>`;
  });
