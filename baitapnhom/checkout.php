...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$order_success) {
  ...
  if (empty($fullname))
    $errors[] = "Vui lòng cung cấp Họ và tên người nhận";
  if (empty($phone))
    $errors[] = "Vui lòng cung cấp Số điện thoại liên hệ";
  if (empty($address))
    $errors[] = "Vui lòng cung cấp Địa chỉ nhận hàng";
    ...
    <?php if ($order_success && $placed_order): ?>
      <div class="card" style="max-width: 750px; margin: 0 auto; padding: 30px;">
        ...
        <div style="background: #f8fafc; border: 1px solid #e5e7eb; border-radius: 12px; padding: 22px;">
          <h3
            style="color: #1e3a8a; font-size: 16px; font-weight: 700; margin-bottom: 14px; border-bottom: 2px solid #cbd5e1; padding-bottom: 6px;">
            📋 THÔNG TIN ĐƠN HÀNG CHI TIẾT</h3>

          <table class="order-detail-table">
            <tr>
              <td>Mã đơn hàng:</td>
              <td style="color: #2563eb !important; font-weight: 700 !important;"><?= $placed_order['order_code'] ?></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td><?= htmlspecialchars($placed_order['fullname']) ?></td>
            </tr>
            <tr>
              <td>Số điện thoại di động:</td>
              <td><?= htmlspecialchars($placed_order['phone']) ?></td>
            </tr>
            <tr>
              <td>Địa chỉ nhận hàng:</td>
              <td><?= htmlspecialchars($placed_order['address']) ?></td>
            </tr>
            <?php if (!empty($placed_order['notes'])): ?>
              <tr>
                <td>Ghi chú đơn hàng:</td>
                <td style="font-style: italic; color: #4b5563 !important; font-weight: normal !important;">
                  <?= htmlspecialchars($placed_order['notes']) ?></td>
              </tr>
            <?php endif; ?>
            <tr>
              <td>Hình thức thanh toán:</td>
              <td style="color: #d97706 !important; font-weight: 700 !important;">
                <?= $placed_order['payment_method'] === 'bank' ? 'Chuyển khoản ngân hàng' : 'Thanh toán khi nhận hàng (COD)' ?>
              </td>
            </tr>
          </table>
          ...
          <?php if ($placed_order['payment_method'] === 'bank'): ?>
            <div class="bank-box">
              <p style="font-weight: 700; color: #1e40af; margin-bottom: 6px; font-size: 14.5px;">🏦 THÔNG TIN TÀI KHOẢN
                CHUYỂN KHOẢN:</p>
              ...
              <div>Số tiền chuyển khoản: <strong
                  style="color: #dc2626; font-size: 16px;"><?= number_format($placed_order['total_all'], 0, ',', '.') ?>đ</strong>
              </div>
              <div>Nội dung chuyển khoản chuẩn: <strong
                  style="color: #2563eb; background: #fff; padding: 2px 8px; border: 1px solid #bfdbfe; border-radius: 4px;"><?= $placed_order['order_code'] ?></strong>
              </div>
              <p style="color: #6b7280; font-size: 12px; margin-top: 8px; font-style: italic;">* Lưu ý: Bạn vui lòng chuyển
                khoản đúng số tiền và ghi chính xác mã nội dung đơn hàng phía trên để hệ thống duyệt đơn tự động.</p>
            </div>
          <?php endif; ?>

          <div style="margin-top: 20px; border-top: 1px dashed #cbd5e1; padding-top: 14px;">
            <p style="font-weight: 700; color: #374151; margin-bottom: 10px; font-size: 14px;">📦 Danh sách sản phẩm mua:
            </p>
            <?php foreach ($placed_order['items'] as $item): ?>
              <div style="display: flex; justify-content: space-between; font-size: 13.5px; padding: 4px 0;">
                <span style="color: #4b5563; max-width: 75%;"><?= htmlspecialchars($item['tenSP']) ?> <span
                    style="color: #9ca3af;">x<?= $item['qty'] ?></span></span>
                <span style="font-weight: 600; color: #1f2937;"><?= number_format($item['subtotal'], 0, ',', '.') ?>đ</span>
              </div>
            <?php endforeach; ?>

            <div
              style="display: flex; justify-content: space-between; font-weight: 700; font-size: 15px; margin-top: 12px; padding-top: 12px; border-top: 1px solid #e5e7eb;">
              <span>Tổng tiền thanh toán đơn hàng:</span>
              <span
                style="color: #dc2626; font-size: 18px; font-weight: 800;"><?= number_format($placed_order['total_all'], 0, ',', '.') ?>đ</span>
            </div>
          </div>
        </div>

        <div style="margin-top: 25px; text-align: center;">
          <a href="?clear_order=1" class="btn btn-success"
            style="display: inline-block; width: auto; padding: 10px 35px;">Tiếp tục mua hàng</a>
        </div>
      </div>
    ...
    <?php else: ?>
      <div class="checkout-layout">
        ...
        <div class="card">
          ...
          <form method="POST" action="checkout.php">
            ...
            <div class="form-group">
              ...
              <input type="text" name="phone" placeholder="Nhập số điện thoại nhận cuộc gọi giao hàng"
                value="<?= htmlspecialchars($_POST['phone'] ?? '') ?>" />
            </div>
            <div class="form-group">
              <label>Địa chỉ nhận hàng chuẩn xác <span style="color:red">*</span></label>
              <input type="text" name="address" placeholder="Số nhà, tên đường, phường/xã, quận/huyện..."
                value="<?= htmlspecialchars($_POST['address'] ?? '') ?>" />
            </div>
            <div class="form-group">
              <label>Ghi chú đơn hàng (Nếu có)</label>
              <textarea name="notes"
                placeholder="Lời nhắn gửi tới cửa hàng hoặc shipper..."><?= htmlspecialchars($_POST['notes'] ?? '') ?></textarea>
            </div>

            <div class="form-group" style="margin-bottom: 18px;">
              <label>Phương thức thanh toán <span style="color:red">*</span></label>
              <div
                style="display: flex; gap: 24px; margin-top: 6px; background: #f8fafc; padding: 12px; border-radius: 9px; border: 1px solid #e5e7eb;">
                <label
                  style="font-weight: 600; font-size: 14px; display: flex; align-items: center; gap: 8px; cursor: pointer; color: #374151;">
                  <input type="radio" name="payment_method" value="cod" checked onchange="toggleFormBankInfo(false)" />
                  COD (Nhận hàng thanh toán)
                </label>
                <label
                  style="font-weight: 600; font-size: 14px; display: flex; align-items: center; gap: 8px; cursor: pointer; color: #374151;">
                  <input type="radio" name="payment_method" value="bank" onchange="toggleFormBankInfo(true)" /> Chuyển
                  khoản ngân hàng
                </label>
              </div>
            </div>

            <div id="form-bank-preview"
              style="display: none; background: #f0fdf4; border: 1px dashed #16a34a; padding: 14px; border-radius: 9px; margin-bottom: 16px; font-size: 13.5px;">
              <p style="font-weight: 700; color: #166534; margin-bottom: 4px;">🏦 Hệ thống chuyển khoản:</p>
              <p>Sau khi nhấn Xác nhận đơn hàng, hệ thống sẽ cấp cho bạn một <strong>Mã đơn hàng riêng biệt</strong> cùng
                thông tin Số tài khoản MB Bank chi tiết để bạn tiến hành quét mã / chuyển khoản.</p>
            </div>
        ...
        <div class="card" style="height: fit-content;">
          ...
          <div style="max-height: 250px; overflow-y: auto; margin-bottom: 15px;">
            <?php foreach ($cart_items as $item): ?>
              <div class="summary-item">
                <span style="max-width: 70%; font-weight: 600; color: #374151;"><?= htmlspecialchars($item['tenSP']) ?>
                  <span style="color: #6b7280; font-weight: normal;">x<?= $item['qty'] ?></span></span>
          ...
          <div
            style="display: flex; justify-content: space-between; align-items: center; font-size: 16px; font-weight: 700; padding-top: 10px; border-top: 2px solid #e5e7eb;">
            <span>Tổng tiền thanh toán:</span>
            <span
              style="color: #dc2626; font-size: 20px; font-weight: 800;"><?= number_format($total_all, 0, ',', '.') ?>đ</span>
          </div>
        </div>
