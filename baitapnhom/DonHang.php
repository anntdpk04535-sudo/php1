...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Tải lại trang và giữ nguyên tham số chi tiết và tìm kiếm nếu đang xem
    $redirectUrl = "DonHang.php";
    $params = [];
    if (isset($_GET['detail']))
        $params[] = "detail=" . urlencode($_GET['detail']);
...
if ($search !== '') {
    $stmt = $pdo->prepare("SELECT * FROM orders WHERE order_id LIKE ? OR fullname LIKE ? ORDER BY order_id DESC");
    ...
    <div class="container">
        <div class="list-box">
            ...
            <form method="GET" style="margin-bottom: 15px; display: flex; gap: 10px;">
                <input type="text" name="search" placeholder="Nhập mã đơn hoặc tên khách hàng..."
                    value="<?= htmlspecialchars($search) ?>"
                    style="flex: 1; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
                <button type="submit"
                    style="padding: 8px 15px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer;">Tìm
                    kiếm</button>
                <?php if ($search !== ''): ?>
                    <a href="DonHang.php"
                        style="padding: 8px 15px; background: #6c757d; color: white; border-radius: 4px; text-decoration: none;">Xóa</a>
                <?php endif; ?>
            </form>
        ...
        <?php if ($detail): ?>
            <div class="detail-box">
                ...
                <p style="margin: 20px 0; padding: 15px; background: #f8f9fa; border-radius: 5px; border: 1px solid #ddd;">
                    <span class="label">Tiến trình:</span><br><br>
                    <?php
                    $status = $detail['status'];

                    if ($status === 'Đã hủy') {
                        echo '<span style="color: red; font-weight: bold;"> Đơn hàng đã bị hủy</span>';
                    } else {
                        $c1 = ($status == 'Đã đặt' || $status == 'Đang giao hàng' || $status == 'Hoàn tất') ? 'color: green; font-weight: bold;' : 'color: gray;';
                        $c2 = ($status == 'Đang giao hàng' || $status == 'Hoàn tất') ? 'color: green; font-weight: bold;' : 'color: gray;';
                        $c3 = ($status == 'Hoàn tất') ? 'color: green; font-weight: bold;' : 'color: gray;';

                        echo '<span style="' . $c1 . '">Đã đặt</span> &nbsp;➔&nbsp; ';
                        echo '<span style="' . $c2 . '">Đang giao hàng</span> &nbsp;➔&nbsp; ';
                        echo '<span style="' . $c3 . '">Hoàn tất</span>';
                    }
                    ?>
                </p>
