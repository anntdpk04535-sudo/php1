...
<body>
  ...
  <form class="product-form" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group"><label>ID sản phẩm</label><input type="text" name="productId"
        value="<?= htmlspecialchars($editProduct['product_id'] ?? '') ?>" <?= $editProduct ? 'readonly style="background: #e9ecef;"' : 'required' ?> /></div>
    <div class="form-group"><label>Mô tả</label><textarea name="description"
        required><?= htmlspecialchars($editProduct['description'] ?? '') ?></textarea></div>
    <div class="form-group"><label>Giá</label><input type="text" name="price"
        value="<?= htmlspecialchars($editProduct['price'] ?? '') ?>" required /></div>
    <div class="form-group"><label>Hình ảnh</label><input type="file" name="image" accept="image/*" <?= $editProduct ? '' : 'required' ?> />
      <?php if ($editProduct && !empty($editProduct['image'])): ?><input type="hidden" name="oldImage"
          value="<?= htmlspecialchars($editProduct['image']) ?>"><img
          src="<?= htmlspecialchars($baseUrl . '/' . ltrim($editProduct['image'], '/')) ?>"
          style="max-width:150px; display:block; margin-top:10px; border-radius:8px;"><?php endif; ?>
    </div>
      ...
      foreach ($products as $product): ?>
        <div class="product-card">
          ...
          <div class="product-info">
            ...
            <div class="btn-group">
              <a href="lab4.php?action=edit&id=<?= urlencode($product['product_id']) ?>" style="flex:1;"><button
                  class="btn-edit" type="button">Sửa</button></a>
            ...
            <a href="cart.php"
              style="display:block; margin-top:10px; text-align:center; background:#16a34a; color:#fff; padding:10px; border-radius:8px; font-weight:bold; text-decoration:none;">🛒
              Xem giỏ hàng &amp; Mua</a>
          </div>
        </div>
      <?php endforeach; else: ?>
  ...
  <div class="modal-overlay" id="detailModal" onclick="if(event.target===this) closeModal()">
    <div class="modal-box">
      <div class="modal-header">
        <h3>Chi tiết sản phẩm</h3><button class="modal-close" onclick="closeModal()">&times;</button>
      </div>
      <div class="modal-body">
        <img id="modal-img" />
        <div class="modal-row"><span class="modal-label">ID:</span><span id="modal-id"></span></div>
        <div class="modal-row"><span class="modal-label">Mô tả:</span><span id="modal-desc"></span></div>
        <div class="modal-row"><span class="modal-label">Giá:</span><span class="modal-value price"
            id="modal-price"></span></div>
      </div>
