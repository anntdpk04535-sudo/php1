...
if (
    ...
    if ($currentOrder) {
        ...
        // Không cho sửa đơn hoàn tất
        if ($currentStatus === 'Hoàn tất') {

            $error = "Đơn hàng [$orderId] đã HOÀN TẤT, không thể thay đổi trạng thái!";

        }
        // Không cho sửa đơn đã hủy
        elseif ($currentStatus === 'Đã hủy') {

            $error = "Đơn hàng [$orderId] đã BỊ HỦY, không thể thay đổi trạng thái!";

        }
        // Không cho quay ngược từ Đang giao hàng về Đã đặt
        elseif (
            $currentStatus === 'Đang giao hàng'
            && $newStatus === 'Đã đặt'
        ) {

            $error = "Không thể chuyển từ 'Đang giao hàng' về 'Đã đặt'!";

        }
        // Không cho quay ngược từ Hoàn tất về trạng thái khác
        elseif (
            $currentStatus === 'Hoàn tất'
            && $newStatus !== 'Hoàn tất'
        ) {

            $error = "Không thể thay đổi đơn hàng đã hoàn tất!";
    ...
    <script>
        ...
        function handleStatusChange(selectElement, orderId) {
            ...
            if (selectElement.value === 'Đã hủy') {
                ...
                reasonInput.required = true; // Bắt buộc phải nhập trên trình duyệt
            } else {
        ...
        <div class="card">
            <table>
                ...
                <?php foreach ($orders as $o):
                    ...
                    // Khóa kiểm soát form nếu đơn đã kết thúc (Hoàn tất hoặc Hủy)
                        ...
                        <td>
                            <form method="POST"
                                ...
                                <div style="display:flex; gap:5px;">
                                    <select name="status" onchange="handleStatusChange(this, '<?= $o['order_id'] ?>')"
                                        <?= $isLocked ? 'disabled' : '' ?>>
                                        <option value="Đã đặt" <?= $o['status'] === 'Đã đặt' ? 'selected' : '' ?>>Đã đặt
                                        </option>
                                        <option value="Đang giao hàng" <?= $o['status'] === 'Đang giao hàng' ? 'selected' : '' ?>>Đang giao hàng</option>
                                        <option value="Hoàn tất" <?= $o['status'] === 'Hoàn tất' ? 'selected' : '' ?>>Hoàn tất
                                        </option>
                                        <option value="Đã hủy" <?= $o['status'] === 'Đã hủy' ? 'selected' : '' ?>>hủy đơn
                                        </option>
                                    </select>
                                ...
                                <div class="reason-box" id="reason_box_<?= $o['order_id'] ?>">
                                    <input type="text" name="cancel_reason" id="reason_input_<?= $o['order_id'] ?>"
